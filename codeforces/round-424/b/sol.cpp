...
/// Template (una parte horrible del código)
...
int main(){
    ...
    /// es la posición donde esta ocurre.
    sort(B, B + n);
    ...
    /// y ahí es donde uso el BIT. 

    /// ...

    /// En esta primera parte del código lo que hago es calcular el orden
    /// en que voy a eliminar las cartas, fijate que el orden lo estoy
    /// guardando en el array A.

    /// En la variable cur guardo la última posición eliminada (fijate) que
    /// yo enumero las posicines desde 1 así que iniciar con 0 está bien.
    /// La idea de esta variable es que si deseo eliminar un conjunto de números
    /// iguales esto los voy a borrar de forma consecutiva pero quizás rotados,
    /// pues tengo que empezar a borrar a partir del último elemento eliminado.

    /// pnt es para saber cuantos elementos he insertado hasta ahora en A.
    /// fijate que solo lo uso en la línea """A[ pnt++ ] = ..."""

    int cur = 0, pnt = 0;

    for (int i = 0, j; i < n; i = j){
        /// Este doble for que ves aquí es lineal. Por si no lo has visto
        /// antes te comento lo que hace. Fijo con el valor i la posición
        ...
        /// encontrar la última posición en B con el mismo valor. Fijate que la
        /// condición del <for> superior no es i++ como suele ser, sino i = j,
        /// para empezar a partir del próximo valor diferente.
        for (j = i; j < n && B[j].first == B[i].first; ++j);

        /// Aquí proceso todos los elementos del mismo valor.
    ...
    /// Si ya tenemos guardada la permutación A, como borrar los elementos de forma
    /// eficiente. La idea es muy simple, si el último elemento que borramos es el 
    ...
    /// ciclicos. O sea entre 2 y 5 están los elementos [2, 3, 4, 5] pero entre 5 y 2
    /// están los elementos [5, 6, ... n, 1, 2]. Debemos saber contar cuantos elementos
    ...
    for (int i = 0; i < n; ++i){
        ...
        /// elemnto u y el último elemento borrad es el
        /// elemento p.
        int u = A[i];
        ...
        update(u, -1);
        /// Y guardamos quien fue el último elemento procesado que es desde
        /// donde nos debemos mover ahora.
        p = u;
    }

    /// Y la parte más feliz del código ;)
    cout << answer << endl;
